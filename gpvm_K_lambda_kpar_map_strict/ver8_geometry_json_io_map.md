# ver8: geometry JSON の生成・参照マップ（__file__ 基準 / cwd 非依存）

対象: `ver8.tar` を展開したディレクトリ（以下ではそれを **REPO_ROOT** と呼びます）
...
を、**ver8 の実コードに基づいて**1本ずつ明記します。

---

## 結論（重要）

ver8 の主要スクリプトは **全て `__file__`（=スクリプト自身の場所）基準**で
...
**要点**
- ver8 で `best_geometry.json` を **生成/上書きするのはこの1本だけ**です。
...
## 3) 重複配置による事故ポイント（ver8 で “混乱の温床” になっている箇所）

ver8 には **同名の heatmap スクリプトが2箇所**に存在します：

1. **ルート側（正）**  
   `REPO_ROOT/gpvm_K_lambda_kpar_map_strict/run_gpvm_K_lambda_kpar_map_strict.py`  
   → `REPO_ROOT/out_gpvm_step2/best_geometry.json` を読む（ver8 の実体と整合）
...
**対策**
- ver8 では基本的に **ルート側のスクリプト群**を使う（`REPO_ROOT/gpvm_*` や `REPO_ROOT/run_*`）
...
という設計に整理できます（ver9以降の混乱もここが根本原因になりがちです）。
