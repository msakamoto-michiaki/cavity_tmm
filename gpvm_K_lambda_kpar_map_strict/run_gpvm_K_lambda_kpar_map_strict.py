...
def fwhm_nm(lam_nm: np.ndarray, y: np.ndarray) -> Tuple[float, float, float, float]:
    """Return (peak_nm, fwhm_nm, left_nm, right_nm) for a 1D spectrum.

    Robust to NaNs: if y has no finite positive values, returns NaNs.
    """
    ...
    def lerp_x(x0, x1, y0, y1, y_target):
        if y1 == y0:
            return float("nan")
        return float(x0 + (x1 - x0) * (y_target - y0) / (y1 - y0))

    left_nm = lerp_x(lam_nm[i0], lam_nm[i1], y_norm[i0], y_norm[i1], half)
    right_nm = lerp_x(lam_nm[j0], lam_nm[j1], y_norm[j0], y_norm[j1], half)

    if (not np.isfinite(left_nm)) or (not np.isfinite(right_nm)) or (right_nm <= left_nm):
        return peak_nm, float("nan"), float("nan"), float("nan")
...
def desired_lam_grid_nm() -> 'np.ndarray':
    ...
    # Override min/max if explicitly provided
...
def make_figures(data: Dict[str, np.ndarray]) -> None:
    ...
    for name, Z, Ku0, ylabel in check_items:
        ...
        if np.isfinite(peak_nm):
            ax.axvline(peak_nm, linestyle=":", linewidth=1)
        else:
            ax.text(0.02, 0.95, "no finite peak", transform=ax.transAxes, va="top")
